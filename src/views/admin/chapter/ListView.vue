<template>
        ...
        <div class=" overflow-x-auto">
          <div class="min-w-full inline-block align-middle">
            <div class="relative  text-gray-500 focus-within:text-gray-900 mb-4">
              <div class="">
                  <h1 class="text-3xl font-semibold text-gray-800">Chapters</h1>
              </div>
                <div class="flex items-center justify-between bg-gray-100 py-4 px-6 rounded-md shadow-sm ">
                  <div>
                    <h4 class="text-2xl font-semibold text-gray-800">Chapter List</h4>
                    <p class="text-sm text-gray-500">Manage your chapter content here.</p>
                  </div>
                  <div class="mt-4 sm:mt-0">
                    <router-link to="/admin/chaptercreate" class="bg-emerald-600 text-white text-sm font-semibold px-4 py-2 rounded-md shadow-sm hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                    Add New Chapter
                  </router-link>
                  </div>
                </div>
                  ...
                  <table class=" min-w-full rounded-xl">
                      <thead>
                          <tr class="bg-gray-50">
                              ...
                              <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Course</th>
                              <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize rounded-t-xl"> Actions </th>
                          </tr>
                      </thead>
                      <tbody class="divide-y divide-gray-300" v-for="data in chapterData" :key="data.id">
                          <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                              <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 ">{{ data.id }}</td>
                              <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ data.name.length > 60 ? data.name.slice(0,60) + '...' : data.name }} </td>
                              <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {{ data.course_name }}</td> 

                              <td class=" p-5 ">
                                  <div class="flex items-center gap-1">
                                      <router-link to="/admin/chapteredit" @click="storeSessionId(data.id)"
                                      ...
                                      /
                                      <button @click="deleteChapter(data.id)" class="p-2 rounded-full  group transition-all duration-500  flex item-center">
  ...
  export default {
    ...
    setup() {
      ...
      { 
        try{
          const deleteData = await api.delete(`/chapter/delete/${id}`);
          console.log(deleteData);
          if (deleteData.status === 200) 
          {
            chapterData.value = chapterData.value.filter(data => data.id !== id)
            notification.notify(
              {
          ...
          else 
          {
            notification.notify(
              {
                text:'Error during delete chapter',
                type: 'error'
              }
            );
            notification.notify.close()
        ...
        {
          console.log('Chapter deleting error', error);
          notification.notify(
            {
              text:'Error during delete chapter',
              type: 'error'
      ...
      onMounted(() => {
        fetchchapterData(); // Call fetch function when component mounts
      });
