<template>
                          ...
                          <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                              ...
                              <td class=" p-5 ">
                                  <div class="flex items-center gap-1">
                                      <router-link to="/admin/courseedit" @click="storeSessionId(data.id)"
                                      ...
                                      /
                                      <button @click="deleteCourse(data.id)" class="p-2 rounded-full  group transition-all duration-500  flex item-center">
  ...
  export default {
    ...
    setup() {
      ...
      { 
        try{
          const deleteData = await api.delete(`/course/delete/${id}`);
          console.log(deleteData);
          if (deleteData.status === 200) 
          {
            courseData.value = courseData.value.filter(data => data.id !== id)
            notification.notify(
              {
                text:'Course is successfully deleted.',
                type: 'warn'
              }
            );
      ...
      onMounted(() => {
        fetchcourseData(); // Call fetch function when component mounts
      });
